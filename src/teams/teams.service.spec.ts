...
describe('TeamsService', () => {
  ...
  describe('create', () => {
    it('should throw ConflictException if team code already exists', async () => {
      mockPrismaService.team.findUnique.mockResolvedValue({ id: 't1', code: 'TEST' });
      await expect(service.create({ name: 'Test Team', code: 'TEST' })).rejects.toThrow(ConflictException);
    });
  ...
  describe('update', () => {
    it('should throw ConflictException if updating to an existing code', async () => {
      ...
      await expect(service.update('t1', { code: 'NEW' })).rejects.toThrow(ConflictException);
    });

    it('should update the team successfully', async () => {
      const team = { id: 't1', code: 'OLD' };
      mockPrismaService.team.findUnique.mockResolvedValueOnce(team).mockResolvedValueOnce(null);
      mockPrismaService.team.update.mockResolvedValue({ id: 't1', code: 'NEW' });
  ...
  describe('remove', () => {
    it('should remove the team', async () => {
      ...
      expect(mockPrismaService.team.delete).toHaveBeenCalledWith({ where: { id: 't1' } });
    });
  });
  ...
  describe('setCaptain', () => {
    it('should throw NotFoundException if player not found in team', async () => {
      ...
      await expect(service.setCaptain('team1', 'player1', true)).rejects.toThrow(NotFoundException);
    });
